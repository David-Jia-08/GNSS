...
if (fid > 0)
    ...
    samplesPerCode = round(settings.samplingFreq /1000);
    ...
    % records).
    fseek(fid, settings.skipNumberOfMs * samplesPerCode * dataAdaptCoeff, 'bof');



    blockCnt = 90*48;
    % Read 100ms of signal
   [data, count] = fread(fid, [1, dataAdaptCoeff*blockCnt*samplesPerCode], settings.dataType);


    fclose(fid);

    if (count < dataAdaptCoeff*blockCnt*samplesPerCode)
